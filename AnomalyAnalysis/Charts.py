...
def timeLine(df,nome_var):
    ...
    buffer.seek(0)
    # Codificar o conteúdo do buffer em Base64
    img_base64 = base64.b64encode(buffer.getvalue()).decode('utf-8')

    plt.clf()
    #Calcular quantos pontos estÃ£o dentro ou fora de 1 desvio padrao
    min = mean -1*std_dev
    max = mean + 1*std_dev 
...
def timelineOficial(df2,nome_var):
    ...
    #Verificar as maiores variaÃ§Ãµes
    aux = df.sort_values(by=['CHANGE_STD'], ascending=False)
    #print(aux.head(10))
    ...
    # Formatar os rÃ³tulos de data para mostrar horas
    ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d %H:%M'))
    ax.xaxis.set_major_locator(mdates.HourLocator(interval=1))

    # Rotacionar e alinhar os rÃ³tulos de data
    fig.autofmt_xdate()

    # Adicionar espaÃ§o extra para os rÃ³tulos dos ticks
...
def boxplot(df,nome_var):
    ...
    # Calculando a mÃ©dia para cada grupo
    # Calcular mÃ©dia e desvio padrÃ£o
    stats = df.groupby('CLUSTER')['PV'].agg(['mean', 'std','count']).T
    stats = stats.round(2)
    # Adicionar tabela com mÃ©dia e desvio padrÃ£o
    table_data = stats.values
    row_labels = ['Media', 'Desvio Padrao','Tamanho']
...
def boxplot2(df,nome_var):
    ...
    # Adicionar tabela com mÃ©dia e desvio padrÃ£o
    table_data = stats.values
    row_labels = ['MÃ©dia', 'Desvio PadrÃ£o']
    table = plt.table(cellText=table_data, rowLabels=row_labels, colLabels=stats.columns, 
...
def fatoresAVG(df,nome_var):
    ...
    # Adicionar anotaÃ§Ã£o no campo superior direito
